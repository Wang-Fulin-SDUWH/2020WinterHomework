...
static double l_rate=0.01;
static int n_epoch=100;
//公共部分函数声明（CSV读取与交叉检验）
...
double *coef_sgd(double **train, double l_rate, int n_epoch)
{
    ...
    for(int epoch=0;epoch<n_epoch;epoch++)
    {
        ...
        for(int j=0;j<(n_folds-1)*fold_size;j++)
        {
            ...
            sumerr+=pow(error,2);
            coef[0]=coef[0]-l_rate*error*yhat*(1.0-yhat);//更新截距
            for(int k=0;k<col-1;k++)
            {
                coef[k+1]=coef[k+1]-l_rate*error*train[j][k]*yhat*(1.0-yhat);//更新相关系数
            }
        }
